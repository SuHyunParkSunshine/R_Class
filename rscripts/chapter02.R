...
#어떤 숫자를 모아놓은 일렬의 숫자, 열 우선!! 가로 노노 세로 예스예스
c(1:20)
1:20 #1포함, 20포함/ 파이썬의 슬라이싱과 유사하게 생김
c(1, 2, 3, 4, 5)

# 실습: seq() 함수를 이용한 벡터 객체 생성
seq(1, 10, 2) #1~10까지 2칸씩

# rep() 함수를 이용한 벡터 생성
rep(1:3, 3) #1~3까지 3번
rep(1:3, each = 3) #각각 3번씩
...
# 실습: 숫자형, 문자형 논리형 벡터 생성(동일한 데이터 타입을 가지고 리스트를 만듬)
v1 <- c(33, -5, 20:23, 12, -2:3)
v2 <- c("홍길동", "이순신", "유관순")
v3 <- c(T, TRUE, FALSE, T, TRUE, F, T) #T 함부로 변수 선언 하지 않기!!!
v1; v2; v3

# 실습: 자료형이 혼합된 경우 
v4 <- c(33, 05, 20:23, 12, "4") #for loop 안 돌아감 들어가면 모두 문자열이 됨.
v4
...
# 실습: ;apply() 함수 적용하기 
apply(x, 1, max) #1은 column(열) 2는 row(행)
apply(x, 1, min)
apply(x, 2, mean)
...
result <- apply(x, 1, f) #f 내가 만든 함수!! python의 list comprehensive를 꼭 쓸 수 있도록 해야댐!!(책에 있는 if, for문을 list comprehensive로 변경할 수 있는 정도는 되야함.)
result
...
getwd()
txtemp <- read.table('data/emp.txt', header = 1, sep = "", fileEncoding = "euc-kr") #cp 949 error는 한글 문제, euc-kr이 문제
txtemp
...
x1 <- subset(df, x >= 3) #'x >= 3' => 값이 boolean으로 나옴, subset = python's ??
x1
y1 <- subset(df, y <= 8)
...
# 실습: key와 value 형식으로 리스트 객체 생성하기 => python's dictionary
...
library(stringr)
str_extract("홍길동35이순신45유관순25", "[1-9]{2}") #1~9까지 숫자 2개 뽑아주세요. "[1-9]{2}"=>정규식
...
str_extract_all(string, "[가-힣]{3}") #"[가-힣]" => 한글 다 가지고 오는 거
str_extract_all(string, "[a-z]{3}")
