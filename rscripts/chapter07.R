...
# 실습: sum() 함수의 속성을 이용하여 결측치 제거(na.rm = T)하기 
...
# 실습: 범주형 변수의 극단치 처리하기 
table(dataset$gender) #빈도수!! -> frequency
pie(table(dataset$gender))


# 실습: subset() 함수를 사용하여 데이터 정제하기(정제=극단치를 제거한 것)
...
densityplot(~ price | factor(gender), #factor를 하면 한 화면에 그래프가 2개 나옴.
            data = new_data, 
            groups = position2, 
            plot.points = T, auto.key = T)
...
# 단계 1: 표본 추출하기 
nrow(data)
choice1 <- sample(nrow(data), 30) #400개 레코드에서 뽑을 30개를 뽑음, 행의 번호를 뽑았다.
choice1

# 50 ~ (data 길이) 사이에서 30개 행을 무작위 추출
choice2 <- sample(50:nrow(data), 30) #50~400사이의 수를 30개 뽑았다.
choice2
