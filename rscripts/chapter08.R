...
# 단계 1: depth 변수 리코딩 -> 새로운 columns를 만들건데, 범주를 나눠서 만들거다!
#depth3를 만들건데 '[]'는 조건이고 d1의 값을 넣을 것이다.
...
# 단계 3: factor 형으로 변환(factor = column에 값을 넣어주는 것)
...
# 단계 1: 조건의 구간 막대가 0.1 단위(10%)로 겹쳐 범주화 
...
#cloud(행 ~ 열(위도 * 경도)) / depth = 깊이, lat = 위도, long = 경도
...
# 단계 2: fill 속성 적용/ fill =  그래프에서 특정 요소의 색상을 지정하는데 사용
