...
# 실습: 구성 비율 계산
prop.table(x) #proportion(비율) - prop. 비율계산 
y <- prop.table(x)
round(y * 100, 2) #빈도수에 백분율을 적용하여 비율을 계산
...
str(x.m)
str(x.t)
which(x.m[1, ] == 2) #행렬(1행)에서 나머지 column에 대해서 빈도수가 2인 것의 인덱스 번호가 나옴
which(x.m[1, ] == 5)

# 단계 3: 데이터프레임으로 변경
x.df <- as.data.frame(x.m)
which(x.df[1, ] == 18) #빈도수가 18인 색인(index) print
...
# 단계 4: 연속형 변수 범주화 => 7단계를 3단계로 줄였다(데이터 전처리)
...
attach(data)
length(cost)
length(data$cost) # => attach를 사용하면 'data$'안 적어도 됨 -> 그냥 length(cost)만 해도 됨.
summary(cost)
mean(cost)
min(cost)
max(cost)
