...
# 단계 4: 규칙 발견
rule <- apriori(tran, parameter = list(supp = 0.3, conf = 0.1)) #apriori 규칙을 만드는 함수
inspect(rule)

# 단계 5: 규칙 발견
rule <- apriori(tran, parameter = list(supp = 0.1, conf = 0.1))
rule <- apriori(tran, parameter = list(supp = 0.5, conf = 0.5))
inspect(rule)
...
plot(rules, method = "grouped", engine = "htmlwidget")
